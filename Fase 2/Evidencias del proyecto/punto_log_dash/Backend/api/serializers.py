...
class OrderCreateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        # If envio, address must be present
        if data["delivery"]["mode"] == "envio" and not (data["delivery"].get("address") or "").strip():
            raise serializers.ValidationError("Dirección requerida para envío.")
        return data
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ["id", "code", "created_at", "full_name", "phone", "delivery_mode", "address", "payment_method", "total", "items"]
